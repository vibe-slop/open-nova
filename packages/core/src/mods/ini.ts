 ...
 * of the original Nova Chrysalia `modconfig.ini` (which wrapped the Win32
 * `GetPrivateProfileString` / `WritePrivateProfileString` APIs).
 *
 ...
 * leading space after the `=` is stripped from each value (the original always
...
export function parseIni(text: string): IniData {
  ...
  for (const rawLine of lines) {
    ...
    // Tolerate the original's single leading space after '='.
 ...
 * emitted as `Key= value` (note the LEADING SPACE) to match the original tool's
 * byte output exactly. Sections are separated by a blank line.
 */
...
/**
 * Interpret an INI value as a boolean using the original's `ReadBool` rule:
 * `undefined`/empty → `false`; otherwise `true` only when the value is a
 * case-insensitive `"true"`, else `false`.
 */
...
export class Ini {
   ...
   * is case-sensitive, matching the original.
   */
  ...
  /** Get a value as a boolean using {@link parseBool} (`ReadBool`) semantics. */
