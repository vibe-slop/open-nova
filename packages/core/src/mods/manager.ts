 ...
 * ModPacks, as described in docs/REVERSE-ENGINEERING.md §3.
 *
 * The original tracked NO database: state lives entirely in each mod's
 * `modconfig.ini` flags plus a per-game `Backup/` folder. This reimplementation
 * keeps that design but is fully cross-platform (pure `fs` + `path.join`, no
 * Windows separators or APIs).
 *
 ...
 * etc.`, detected via `_`-prefixed folder names in the unpacked tree) need the
 * container unpack/repack path from §3 step 4. That is intentionally NOT done
 * here — loose-file overlay covers most of the Nexus catalogue for the MVP.
 * See the `TODO(wpd)` below.
 */
...
/** Deliberately MISSPELLED — preserved for interop with the original tool. */
...
export class ModManager {
  ...
  async generateModPack(spec: ModPackSpec, ncmpPath: string): Promise<void> {
    ...
    try {
      ...
      // Misspelled state section, preserved for interop.
   ...
   * original unpacks/repacks the surrounding WPD via `!!WPD_Records.txt`. That
