 ...
 *
 * FF13 mods have no standard manifest, so we infer the layout:
 *   - a `.ncmp` style pack (has modconfig.ini + Data/...),
...
export async function detectMod(extractedDir: string): Promise<DetectedMod> {
  ...
  //    folder holding `.dds` files to inject (the `_<container>` naming convention,
  ...
  // 1) .ncmp style pack: modconfig.ini + overlay dirs.
