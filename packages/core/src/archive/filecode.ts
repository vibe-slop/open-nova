 ...
 * any per-mod data.
 *
 * The layout reproduces 9,085/9,085 non-stripped `chr/pc` codes in a real
 * FFXIII-2 filelist (the 30 it "misses" are exactly the repointed DLC slots).
 *
...
export function computeFileCode(virtualPath: string, game: GameCode): ComputedCode | null {
  // A single hardcoded special case: the DLC key file has a fixed code.
