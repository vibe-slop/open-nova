 ...
 *
 * See docs/REVERSE-ENGINEERING.md §1. Self-consistency tested via round-trip;
 * byte-identity vs the original tool must be confirmed against a real install.
 */
...
export function buildFilelist(fl: Filelist): Buffer {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (gameCode === 1) {
      ...
      // Odd-indexed chunks carry the 0x8000 flag (matches original scheme).
