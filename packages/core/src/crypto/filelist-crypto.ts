 ...
 *
 * NOTE: the block cipher is validated byte-for-byte against the original DLL
 * (test/crypto.vectors.test.mjs). This framing layer is implemented from the
 * decompiled spec and should be validated against a real game filelist before
 * relying on the re-encrypt path in production.
 */
 ...
 * Pads the body to an 8-byte multiple and writes the size fields + checksum
 * exactly as the original EncryptProcess + ProcessFilelist('e') do.
 */
export function encryptFilelist(plain: Uint8Array): Uint8Array {
  // Preprocess (FilelistCrypto.EncryptProcess): pad body to 8, append 16 nulls,
  // write body size BE @ 0x10 and LE @ (length-16).
