/**
 * Final Fantasy XIII / XIII-2 / Lightning Returns "filelist" block cipher.
 *
 ...
 *
 * The S-box is the identity-plus-120 permutation:
 * Integers[i] = (i + 120) mod 256.
 */
 ...
 * The seed is reversed during derivation; we operate on a copy so the
...
export function encryptBlocks(xorTable: Uint8Array, data: Uint8Array, blockCount: number): Uint8Array {
  ...
  for (let i = 0; i < blockCount; i++) {
    ...
    // each sign-extended in the high 32 bits with 0xFFFFFFFF.
...
function beU32(a: Uint8Array, lo: number, hi: number): number {
  // The low 32 bits are the bytes a[hi],a[hi-1],...,a[lo] in big-endian order.
  let v = 0;
