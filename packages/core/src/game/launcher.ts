 ...
 *
 * The original Nova Chrysalia applied these as LIVE process-memory writes to
 * the running game (at module base + offset, i.e. by RVA). That doesn't work
 * under Proton on Linux, so open-nova applies the SAME edits to the on-disk
 * executable before launch (mapping each RVA to a file offset via the PE
 * section table) and keeps a `.original` backup. The bytes are taken verbatim
 * from GameLauncher.cs.
 *
