 ...
 * Node `Buffer`. These REPLACE the original tool's live process-memory patching
 * (`WriteProcessMemory` into a suspended game process at fixed module-relative
 * offsets) which does not port to Linux / Proton.
 *
 * The portable substitute described in docs/ARCHITECTURE.md is: edit the SAME
 * bytes in the on-disk executable BEFORE launch (mapping each RVA to a file
 * offset via the PE section table), keep a `.original` backup, and launch the
 * game through `steam://rungameid/<appid>` so Steam/Proton wraps it.
 *
 ...
 *
 * The concrete RVAs for unpacked-mode / text-language / debug patches (the ones
 * the original `GameLauncher` wrote) are GAME-SPECIFIC and belong in
 * game/launcher.ts, which is DEFERRED. launcher.ts will also own the
 * `.original` backup/restore lifecycle and the `steam://rungameid/<appid>`
 * launch. None of those RVAs are hardcoded here — only the generic machinery.
 *
 ...
 * which the original applied to avoid out-of-memory crashes with large mods.
 *
 ...
 *
 * This is the on-disk equivalent of the original's module-base + RVA addressing
 * used by the live-memory patcher.
 */
