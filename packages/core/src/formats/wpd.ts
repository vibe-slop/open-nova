 ...
 *   0x00  'WPD\0'              magic (4 bytes; "WPD" followed by NUL
 *                              padding through to the record table @0x10)
...
/** Data bodies are padded to this alignment. */
const DATA_ALIGN = 4;
...
export interface WpdEntry {
   ...
   * when the record stores no extension (a lone "." means none).
   */
  ext: string;
...
export function unpackWpd(buf: Uint8Array | Buffer): UnpackedWpd {
  ...
  // Magic: read 4 bytes and compare the leading text to "WPD".
  ...
  for (let i = 0; i < recordCount; i++) {
    ...
    // A lone "." in the ext field means "no extension"; here an absent
    // extension is simply the empty string.
 ...
 * and the 4-byte body alignment. Names and extensions are encoded as
