 ...
 *
 * The original derives region starts arithmetically:
 ...
 * and treats the last two descriptors (indices resCount-2 and resCount-1) as the
 ...
 * RESOURCE_TYPE / RESOURCE_ID side files the original emits on unpack, plus the
 * IMGB pairing path. {@link unpackTrb} reproduces those regions in the returned
 * structure; {@link repackTrb} is a documented best-effort rebuild that must be
 * validated against a real install (see docs/ARCHITECTURE.md milestone 2).
 */
...
const DESC_SIZE = 16;
/** Bodies are padded to this alignment, matching the original. */
const DATA_ALIGN = 4;
...
export interface TrbEntry {
  ...
  name: string;
  /**
   * 4-character resource type tag (from the RESOURCE_TYPE region). The original
   * reverses the 4 bytes when reading, so this is the human-readable form (e.g.
   * "txbh"). Empty for the final descriptor which has no type tag.
   */
  type: string;
...
export interface UnpackedTrb {
  ...
  /**
   * The complete header + descriptor table region (offsets 0 .. tableEnd). This
   * is the original's `SEDBRES_OFFSETS` side file — preserved verbatim so a
   * faithful rebuild can reuse it.
   */
  offsetsRegion: Buffer;
}

/**
 * Unpack a TRB ("SEDBRES ") container. Follows the original's region arithmetic
 * exactly: the descriptor table is read at offset 64, region bases are derived
 * from the directory-data-length field and the last descriptors, and each real
 * resource body is sliced by its (offset, size) where the size of the
 * last-but-one and last real resources is computed from the following region.
 *
...
export function unpackTrb(buf: Uint8Array | Buffer): UnpackedTrb {
  ...
  const dirDataLen = r.readU32(false); // "num"
  const resCount = r.readU32(false); // "num2" (read immediately after)

  const tableEnd = DESC_TABLE_START + resCount * DESC_SIZE; // "num3"
  const lastIdx = resCount - 1; // "num4"

  // Offsets of the two region descriptors (RESOURCE_TYPE & RESOURCE_ID).
  // descriptor[resCount-2].dataOff (the +4 field) -> typeRegion size base "num5"
  const typeRegionRel = new BinaryReader(
    r.buf,
    DESC_TABLE_START + (resCount - 2) * DESC_SIZE + 4,
  ).readU32(false); // "num5"
  // descriptor[resCount-1].dataOff -> used to bound the last-but-one body "num6"
  const idRegionRel = new BinaryReader(
    r.buf,
    DESC_TABLE_START + lastIdx * DESC_SIZE + 4,
  ).readU32(false); // "num6"

  const namesBase = tableEnd + dirDataLen; // "num8" -> RESOURCE_ID strings
  const typesBase = tableEnd + typeRegionRel; // "num9" -> RESOURCE_TYPE tags

  const entries: TrbEntry[] = [];
  let descPos = DESC_TABLE_START + 4; // "num7": +4 lands on the dataOff field
  let nameCursor = namesBase;
  let typeCursor = typesBase;

  // The original loops num12 = 1 .. resCount (inclusive). Index i = num12.
  for (let i = 1; i <= resCount; i++) {
    ...
    if (i < lastIdx) {
      ...
      // 4-char type tag, byte-reversed (shouldReverse: true in the original)
 ...
 *
 * The original `RepackTRB` reassembles the file from FOUR side files it wrote on
 * unpack (`SEDBRES_OFFSETS`, `RESOURCE_TYPE`, `RESOURCE_ID`, plus a freshly built
 * body blob) and only rewrites the per-resource offset/size pairs and a handful
 * of header length fields — it never reconstructs the descriptor table or region
 * layout from scratch, and it routes texture resources through IMGBRepack2.
 *
 * This implementation is instead the EXACT INVERSE of {@link unpackTrb}: it
 * consumes the same `entries` array (all `resourceCount` of them, including the
 * final two which carry the RESOURCE_TYPE and RESOURCE_ID region bytes as their
 * `data`), reuses the preserved `offsetsRegion` (header + descriptor table) so
 * reserved/unknown header bytes survive verbatim, lays the real bodies out
 * 4-byte aligned, then appends the RESOURCE_TYPE region followed by the
 * RESOURCE_ID region, and patches every descriptor's offset/size plus the
 * relevant header length fields. By construction, feeding the output back into
 * {@link unpackTrb} reproduces the same structure (round-trip safe).
 *
 * It does NOT perform IMGB texture pairing, and byte-identity with the original
 * tool's output is UNVERIFIED until checked against a real install (see
 * docs/ARCHITECTURE.md milestone 2). Treat this as a correct-by-round-trip
 * rebuild, not a guaranteed-identical one.
 *
