 ...
 *
 * Reassembles the container from the regions captured on unpack
 * (`SEDBRES_OFFSETS` = the header + descriptor table, `RESOURCE_TYPE`,
 * `RESOURCE_ID`, plus a freshly built body blob) and ONLY:
 ...
 *      fixed-formula sizes (`64 + 20*count` and `64 + 32*count`),
 ...
 *
 * The strategy is to keep the header + descriptor table + RESOURCE_TYPE +
 * RESOURCE_ID regions verbatim, rebuild only the data region, and patch the
 * offset table. Because the region-descriptor `size` fields are written from
 * fixed formulas rather than the regions' true byte lengths, preserving them
 * verbatim is what makes a no-edit repack byte-identical to the source file. All
 * offsets/sizes are LITTLE-ENDIAN, matching the TRB container.
 *
 * Scope note: this does NOT perform IMGB texture pairing. It rebuilds the
 * SEDBRES container faithfully from the entries; if a caller has separately
 * repacked a paired `.imgb`, the header blocks held in the entries should
...
const DESC_SIZE = 16;
/** Default body alignment (4-byte pad after each body). */
const DATA_ALIGN = 4;
/**
 * The descriptor `fieldC` value that requests stronger (16-byte) body alignment.
 * Observed on real FFXIII-2 TRBs for `.skl` / `phb` bodies (`fieldC == 4`).
...
export interface RepackTrbInput {
  ...
  /** Header + descriptor table (the `SEDBRES_OFFSETS` region). */
...
/**
 * Repack a TRB ("SEDBRES ") container from parsed entries. Pass the result of
 * {@link unpackTrb} directly (optionally after editing real entries' `data`); if
 * only the raw entries are on hand, pass `originalBuf` so the header +
 * descriptor table can be recovered by re-parsing.
 *
 * Layout produced:
 ...
 * byte-for-byte (validated against a real FFXIII-2 TRB).
 *
...
export function repackTrb(
  ...
  // Recover the header + descriptor table region (the SEDBRES_OFFSETS region).
  ...
  // The real resources are descriptors 0 .. count-3, and the final two
  // (count-2, count-1) are the RESOURCE_TYPE and RESOURCE_ID region descriptors.
  const realCount = resourceCount - 2;

  // Build the body region: each real body is placed at an offset aligned to its
  // resource's requirement, then copied verbatim. Bodies are normally 4-byte
  // aligned; real game files additionally 16-byte-align `fieldC == 4` resources,
  // so we align each body's START to `STRONG_ALIGN` when `fieldC == 4` and
  // `DATA_ALIGN` otherwise. Alignment is computed in ABSOLUTE file space
  ...
  // RESOURCE_TYPE region 4-byte aligned.
  ...
  // The two region descriptors. Their offsets point into the body region and
  // their sizes are the *fixed-formula* values (NOT the regions' true byte
  // lengths) — preserving these formulas is what makes a no-edit repack
  // byte-identical.
  const typeRel = bodies.length; // total aligned body length
  const idRel = typeRel + typeRegion.length; // bodies + TYPE region

  const typeDescBase = DESC_TABLE_START + (resourceCount - 2) * DESC_SIZE;
  head.writeUInt32LE(typeRel >>> 0, typeDescBase + 4);
  head.writeUInt32LE((64 + 20 * resourceCount) >>> 0, typeDescBase + 8);

  const idDescBase = DESC_TABLE_START + (resourceCount - 1) * DESC_SIZE;
  head.writeUInt32LE(idRel >>> 0, idDescBase + 4);
  head.writeUInt32LE((64 + 32 * resourceCount) >>> 0, idDescBase + 8);

  // Header length fields.
  // @0x34 dirDataLen = idRel + count*16. NB this is NOT the same as idRel —
  // unpackTrb's `namesBase = tableEnd + dirDataLen` relies on this exact value to
  // locate the RESOURCE_ID names.
  head.writeUInt32LE((idRel + resourceCount * DESC_SIZE) >>> 0, OFF_DIR_DATA_LEN);
  // @0x38 resource count (unchanged; rewritten so the whole header is consistent).
