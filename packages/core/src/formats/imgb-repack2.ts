 ...
 *   3. for each output mip: compute its size for the format, write start/size
 *      (BE u32, 8 bytes/entry) into the table at GtexStart+24, append the mip's
 *      pixels to a fresh imgb, pad the imgb to a 16-byte minimum per mip, then
 *      halve the working dimensions.
 *
 * Win32 only (PS3/x360 imgb layouts are unsupported). Cubemap (type 1/5) and
 * stack (type 2) are deferred — see {@link repackImgbResize}.
 */
...
/**
 * Mutable working state for the output texture, carried across the per-mip
 * size computation and dimension-halving steps.
 */
interface OutState {
  format: number;
  width: number;
...
/**
 * Halve the working dimensions for the next mip: uncompressed (fmt 3) clamps at
 * 1, everything else clamps at 4.
 */
...
export function repackImgbResize(headerBlock: Buffer, newDds: Buffer): RepackResizeResult {
  ...
  const origMips = g.mipCount; // mip count from the original GTEX (>=1)
  const newMips = dds.mipCount; // mip count of the replacement DDS
  ...
  for (let i = 0; i < newMips; i++) {
    ...
    // Pad the imgb to a 16-byte minimum for this mip.
    if (mipSize < 16) {
    ...
    // Advance the DDS read cursor by the unpadded mip size.
    srcPos += mipSize;
    tablePos += 8;
  }
