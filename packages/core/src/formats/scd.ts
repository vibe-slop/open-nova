 ...
 * absolute offsets, one per sub-song. This implementation reads all
 * `subSongCount` entries.
 *
 ...
 * Vorbis variant via a native tool and is intentionally out of scope here — see
 * NOTE at the bottom.
 */
...
/**
 * SCD stream codec ids. These identify the encoding of each sub-song's raw
 * audio body.
 */
 ...
 * offset 0x0C (0 = little-endian, 1 = big-endian). PC files are little-endian;
 * the FFXIII console builds (PS3/Xbox 360) set this flag for big-endian. As a
 * guard against a corrupt/ambiguous flag, the detected endianness is
 * cross-checked against the `scdSize` field: whichever endianness makes
 * `scdSize` equal the actual file length wins.
 *
 ...
 * each {@link ScdSubSong} (`dataOffset = streamHeaderOffset + 32 +
 * extraHeaderSize`).
 *
 * @param buf the full SCD container bytes
...
export function parseScd(buf: Uint8Array | Buffer): ParsedScd {
  ...
  for (let i = 0; i < subSongCount; i++) {
    ...
    // largest of several SSCF tables); skip it.
...
export interface ExtractedScdStream {
   ...
   * whose raw body is not transcoded.
   */
  name?: string;
...
/**
 * Extract the raw audio sub-streams from an SCD container:
 *
 ...
 *    the audio body; otherwise the raw body is the complete `.ogg`.
 ...
 *  - Any other codec: emits the raw body verbatim with a `.bin` hint, since the
 *    codec is not transcoded but extracting the bytes is still useful.
 *
 * @param buf the full SCD container bytes
...
/**
 * Extract a Vorbis (codec 6) sub-song to a complete `.ogg`. Two paths are keyed
 * on the VorbisEncodedHeader at `streamHeaderOffset + 32`:
 *
 ...
 *
 * The 18-byte `WAVEFORMATEX` plus its `cbSize` extension bytes (`fmt ` chunk)
 * and the raw audio body (`data` chunk) are copied into a standard RIFF/WAVE
 * file. The `fmt ` extension bytes sit immediately after the 18-byte
...
function extractMsAdpcm(data: Buffer, s: ScdSubSong, be: boolean): ExtractedScdStream {
  ...
  const cbSize = hr.readU16(be); // size of the `fmt ` codec-specific extension
 ...
 * Rebuilding an SCD from a source `.ogg` or MS-ADPCM `.wav` requires, for
 * Vorbis, that the source first be re-encoded to Square's non-serialized Vorbis
 * variant using a native tool; a JS-only implementation cannot perform Vorbis
 * encoding. SCD repack is therefore intentionally not implemented in this
 * module.
 */
