 ...
 * byte-identical-shaped ZTR. Port of NovaChrysalia.Formats/ZTR.cs and its
 * ZTRHelpers (Extraction/LinesExtractor + KeysDecoder*, Conversion/PackCmp +
 * PackUncmp + PackIDs + CompressionHelpers + KeysEncoder*, Support/*).
 *
 ...
 *
 * Codepage handling: this port reproduces the codepage-932 ("Latin/Japanese",
 * LJ) decode/encode path, which covers every Western FFXIII release. For that
 * path the message bytes that survive glyph mapping are single-byte and map
 * 1:1 onto cp1252/ASCII, so no external codepage library is required for ASCII
 * text. Full Shift-JIS (cp932), Chinese (cp950) and Korean (cp51949) round
 * trips of non-ASCII source bytes would need a codepage transcoder
 * (`iconv-lite` is the natural optional dependency); see the note in the
 * module's self-test and {@link ZtrLimitations}.
 */
...
export interface BuildZtrOptions extends ZtrOptions {
  /**
   * Byte-pair-compress the IDs and message blobs (the original's default,
   * matching PackCmp). When false the blobs are stored with empty BPE tables
   * (matching PackUncmp). Defaults to true.
   */
  compress?: boolean;
}
...
const MAGIC = 1;
/** Maximum decompressed bytes per BPE chunk (the original's hard limit). */
const MAX_CHUNK = 4096;

// ---------------------------------------------------------------------------
// Byte-pair (BPE) compression — port of CompressionHelpers.cs
// ---------------------------------------------------------------------------

/**
 * The byte values usable as BPE "page" markers: every byte 0..255 that does
 * NOT occur in the data and is not 8 (the original reserves 0x08). Returned in
 * ascending order, matching CompressionHelpers.GetPageNumbers.
 */
...
/**
 * Find the most frequently occurring adjacent byte pair in `data`, scanning the
 * way CompressionHelpers.GetLargestOccuringBytes does (non-overlapping count
 * from the pair's first occurrence). Returns `[a, b, count]`.
 */
 ...
 * `[u32 tableSize][triples][payload]` layout. Port of
 * CompressionHelpers.CompressChunk: repeatedly replace the most common pair
 * with an unused page byte until no pair occurs >= 4 times (or pages run out).
 */
 ...
 * the full byte sequence it stands for. Port of DictionaryHelpers.ArrangeDictChunk.
 */
...
/**
 * Split a stream into <= {@link MAX_CHUNK}-byte groups, matching
 * DictionaryHelpers.GetItemsGroupCount (full 4096-byte groups then a remainder).
 */
...
// ---------------------------------------------------------------------------
// Glyph (control-token) mapping — port of KeysDecoderLJ / KeysEncoderLJ
// ---------------------------------------------------------------------------
...
function decodeLine(bytes: Uint8Array, game: ZtrGameCode): string {
  ...
  while (i < bytes.length) {
    ...
    // 0xFF on its own decodes to {FF} in the original.
    if (b === 0xff) {
      out += '{FF}';
      i++;
      continue;
...
/**
 * Resolve a BaseChara two-byte key to its final glyph token. The original maps
 * the pair to an inner name (e.g. `{0x85_40}`) via BaseCharaKeys, then maps
 * that name (without braces) to the displayed glyph (e.g. `{€}`) via
 * DecodedCharaKeys.
 */
 ...
 * their literal bytes, matching the original's fall-through behaviour.
 */
  ...
  while (i < text.length) {
    ...
    if (ch === '{') {
      ...
      if (close !== -1) {
        ...
        // Unknown token: emit the inner text's literal bytes (the original
        // falls through to writing the brace and inner bytes verbatim).
 ...
 * (matching the original's PackUncmp).
 *
...
export function buildZtr(lines: ZtrLine[], opts: BuildZtrOptions = {}): Buffer {
  ...
  //    chunk). This reproduces the layout the original reader expects.
...
/** Wrap a chunk with an empty BPE table (the PackUncmp `[u32 0][payload]`). */
...
/** Notes on this port's codepage limitations. */
