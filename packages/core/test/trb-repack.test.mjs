 ...
 *
 * The repack ports the original NovaChrysalia TRB.RepackTRB exactly (keep the
 * header + descriptor table + RESOURCE_TYPE + RESOURCE_ID regions verbatim,
 * rebuild only the body region, patch the offset/size table and the @52/@16
 * header fields), so a no-edit repack is byte-identical to the source.
 */
...
if (existsSync(REAL_PATH)) {
  ...
  // dirDataLen @52 = idRel + resCount*16 (matches the original valueToWrite4).
  ...
  // matching the real container's valueToWrite5 = num2 + num13 + num15 + num17.
